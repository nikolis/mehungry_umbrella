...
schema change. Written by `Mehungry.Chemistry.Resolver` (see `docs/science/chemistry.md`).
...
`Food.CompoundCandidates` (**`docs/science/compound_candidates.md`**) derives scored *candidates*
...
context (see **`docs/science/chemistry.md`**), whose `Chemistry.Resolver` is the
...
- No Oban worker or progress-run tracker (unlike the literature crawl in
  `docs/science/literature_discovery.md`) — the registry is a plain layer like
