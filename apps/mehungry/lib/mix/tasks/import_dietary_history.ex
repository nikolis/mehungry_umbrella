defmodule Mix.Tasks.Import.DietaryHistory do
  ...
  professional resolved from `--professional-email`.

      mix import.dietary_history --path FILE.csv --professional-email nutri@example.com
      mix import.dietary_history --path FILE.csv --professional-email nutri@example.com --name "Maria K."
  ...
  def run(args) do
    {opts, _, _} =
      OptionParser.parse(args,
        strict: [path: :string, professional_email: :string, name: :string]
      )
    ...
    import_opts = if opts[:name], do: [full_name: opts[:name]], else: []
