defmodule Mehungry.Literature.AnnotationRuns do
  @moduledoc """
  Lifecycle + progress tracking for PubTator3 annotation passes.

  Mirrors `Mehungry.Literature.CrawlRuns`: a single `AnnotationRun` row carries a
  `processed`/`total` coverage snapshot refreshed each batch, transitions
  ...
  alias Mehungry.Literature.AnnotationRun, as: Run

  @topic "pubtator_annotation_runs"

  def topic, do: @topic

  @doc "Opens a run seeded with the current coverage snapshot and `started_at`."
  def start_run do
    %{processed: processed, total: total} = Literature.annotation_progress()
  ...
  defp update_status(run_id, attrs) do
    case Repo.get(Run, run_id) do
      nil ->
        Logger.debug(
          "Literature.AnnotationRuns: run #{run_id} missing, skipping #{inspect(attrs)}"
        )
  ...
  defp broadcast(%Run{} = run) do
    Phoenix.PubSub.broadcast(Mehungry.PubSub, @topic, {:pubtator_annotation_run, run})
    run
  end
