defmodule Mehungry.Food.NutrientNameNormalizer do
  ...
  defp get_canonical_name(original_name, name_lower) do
    cond do
      ...
      # ===== SATURATED FAT =====
      name_lower in ["fatty acids, total saturated", "total saturated fatty acids", "saturated fat"] ->
        "Saturated Fat"
      String.starts_with?(name_lower, "sfa") ->
        original_name  # Keep SFA as-is for fatty acid identification
      
      # ===== MONOUNSATURATED FAT =====
      name_lower in ["fatty acids, total monounsaturated", "total monounsaturated fatty acids", "monounsaturated fat"] ->
        "Monounsaturated Fat"
      String.starts_with?(name_lower, "mufa") ->
        original_name  # Keep MUFA as-is for fatty acid identification
      
      # ===== POLYUNSATURATED FAT =====
      name_lower in ["fatty acids, total polyunsaturated", "total polyunsaturated fatty acids", "polyunsaturated fat"] ->
        "Polyunsaturated Fat"
      String.starts_with?(name_lower, "pufa") ->
        original_name  # Keep PUFA as-is for fatty acid identification
      ...
      String.contains?(name_lower, "ala") or (String.contains?(name_lower, "18:3") and String.contains?(name_lower, "n-3")) ->
      ...
      # ===== OMEGA-9 FATTY ACIDS =====
      String.contains?(name_lower, "oleic") or (String.contains?(name_lower, "18:1") and not String.contains?(name_lower, "n-7")) ->
        "Oleic Acid (Omega-9)"
      ...
      # ===== MINERALS =====
      name_lower in ["sodium", "na"] -> "Sodium"
      name_lower in ["potassium", "k"] -> "Potassium"
      name_lower in ["calcium", "ca"] -> "Calcium"
      name_lower in ["iron", "fe"] -> "Iron"
      name_lower in ["magnesium", "mg"] -> "Magnesium"
      name_lower in ["phosphorus", "p"] -> "Phosphorus"
      name_lower in ["zinc", "zn"] -> "Zinc"
      name_lower in ["copper", "cu"] -> "Copper"
      name_lower in ["manganese", "mn"] -> "Manganese"
      name_lower in ["selenium", "se"] -> "Selenium"
  ...
  def get_fat_category(name) do
    ...
    cond do
      String.starts_with?(name_lower, "sfa") or 
      (String.contains?(name_lower, "saturated") and not String.contains?(name_lower, "polyunsaturated")) ->
        "Saturated Fat"
