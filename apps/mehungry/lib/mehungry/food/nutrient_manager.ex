defmodule Mehungry.Food.NutrientMerger do
  ...
  def normalize_nutrient_name(name) when is_atom(name), do: normalize_nutrient_name(to_string(name))
  ...
  def find_parent_category(nutrient_name) do
    ...
    # Check for vitamins
    if name_lower =~ ~r/vitamin|retinol|carotene|ascorbic|tocopherol|phylloquinone|thiamin|riboflavin|niacin|pantothenic|pyridoxine|biotin|folate|cobalamin|choline/i do
      cond do
    ...
    # Check for minerals
    if name_lower =~ ~r/calcium|\bca\b|iron|\bfe\b|magnesium|\bmg\b|phosphorus|\bp\b|potassium|\bk\b|sodium|\bna\b|zinc|\bzn\b|copper|\bcu\b|manganese|\bmn\b|selenium|\bse\b/i do
      cond do
  ...
  defp determine_best_unit(group, canonical_name) do
    cond do
      canonical_name == "Energy" -> "kcal"
      String.contains?(canonical_name, "Vitamin") -> "mg"
      true ->
        group
  ...
  defp build_hierarchy_simple(flat_map) do
    ...
    {saturated_list, monounsaturated_list, polyunsaturated_list, trans_list, 
     vitamin_list, mineral_list, sugar_list, other_list} = 
      Enum.reduce(flat_map, {saturated_list, monounsaturated_list, polyunsaturated_list, 
                             trans_list, vitamin_list, mineral_list, sugar_list, other_list}, 
        fn {name, nutrient}, {sat, mono, poly, trans, vita, min, sugar, other} ->
          cond do
            ...
            # Check for minerals
            name in ["Calcium", "Iron", "Magnesium", "Phosphorus", "Potassium", 
                     "Sodium", "Zinc", "Copper", "Manganese", "Selenium"] or
    ...
    result = Enum.reduce(other_list, result, fn nutrient, acc ->
...
defp categorize_remaining(remaining, categories) do
  Enum.reduce(remaining, {categories, []}, fn {name, nutrient}, {acc, other} ->
    cond do
      ...
      String.contains?(name, "Vitamin") or
      String.contains?(name, "vitamin") or
      name in ["Folate", "Choline", "Beta-Carotene", "Retinol", "Ascorbic Acid", "Pantothenic Acid"] ->
        normalized = %{nutrient | "name" => normalize_nutrient_name(name)}
        {%{acc | vitamins: [normalized | acc.vitamins]}, other}
      
      # Check for minerals
      name in ["Calcium", "Iron", "Magnesium", "Phosphorus", "Potassium", "Sodium", 
               "Zinc", "Copper", "Manganese", "Selenium"] or
