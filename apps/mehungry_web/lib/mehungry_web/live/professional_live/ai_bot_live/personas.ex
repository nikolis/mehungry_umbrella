  ...
  def render(assigns) do
    ...
    <div class="flex gap-6">
      <div class="w-96 flex-shrink-0 space-y-2">
          ...
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <div class="font-semibold text-white text-sm flex items-center gap-2">
                ...
                <span
                  :if={!persona.active}
                  class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-ink-panel2/50 text-parchment-dim border border-ink-panel2/40"
                >
                  Off
                </span>
              </div>
        ...
        <p :if={@personas == []} class="text-sm text-parchment-dim">No personas yet.</p>
      </div>
      ...
      <div :if={@form} class="flex-1 min-w-0">
        <div class="bg-ink-panel border border-ink-panel2/60 rounded-xl p-6">
          ...
          <.form for={@form} phx-change="validate" phx-submit="save" class="space-y-4">
            ...
            <div>
              <label class="block text-xs text-parchment-dim mb-1">Archetype (optional)</label>
              <.input
            ...
            <div>
              <label class="block text-xs text-parchment-dim mb-1">Short description (optional)</label>
              <.input field={@form[:description]} type="text" class={input_class()} />
            </div>
            <div>
              <label class="block text-xs text-parchment-dim mb-1">Voice prompt</label>
              <.input
            ...
            <div>
              <label class="block text-xs text-parchment-dim mb-1">Default origin (optional)</label>
              <.input
