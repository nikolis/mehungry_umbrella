  ...
  def render(assigns) do
    ...
    <div>
      ...
      <div class="flex items-center gap-3 mb-5">
        <.link navigate={~p"/professional/ai-bot/review/#{@bot_recipe.id}"}
        ...
        <button phx-click="retranslate" disabled={@translating}
          ...
          <.form for={@form} phx-change="validate" phx-submit="save_translation" class="space-y-3">
            ...
            <div class="bg-slate-800 border border-slate-700/60 rounded-xl p-4 space-y-3">
              ...
              <div>
                ...
                <textarea name="recipe_translation[description]"
                          ...
                          rows="5"><%= Phoenix.HTML.Form.input_value(@form, :description) %></textarea>
              </div>
            </div>
                  ...
                  <%= for {step, i} <- Enum.with_index(@recipe.steps) do %>
                    <div>
                      <label class="flex items-center gap-1.5 text-xs text-slate-500 mb-1">
                        <span class="w-4 h-4 rounded-full bg-slate-700 text-slate-400 text-[10px] font-bold flex items-center justify-center flex-shrink-0"><%= i + 1 %></span>
                        Step <%= i + 1 %>
                      </label>
                      <textarea name={"recipe_translation[steps][#{i}]"}
                                rows="3"
                                class="w-full bg-slate-700 border border-slate-600 rounded-lg text-slate-200 text-sm px-3 py-2 focus:border-primary-500 focus:outline-none resize-none"
                                placeholder={step.description}><%= get_step_translation(@form, i) %></textarea>
                    </div>
                  <% end %>
                </div>
              </div>
