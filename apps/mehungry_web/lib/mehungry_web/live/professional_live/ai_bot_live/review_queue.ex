defmodule MehungryWeb.AiBotLive.ReviewQueue do
    ...
    case DailyRecipeGenerationWorker.new(%{target_date: target_date}) |> Oban.insert() do
      {:ok, _} ->
        {:noreply,
         ...
         |> put_flash(:info, "Generation job queued for #{target_date} — recipes will appear here shortly.")}
  ...
  def handle_event("import_one", %{"id" => recipe_id}, socket) do
    ...
    if is_nil(config) do
      {:noreply, put_flash(socket, :error, "No active bot config for this month — create one first.")}
    else
      ...
      case AiBot.import_single_recipe(String.to_integer(recipe_id), config, scheduled_date) do
        ...
        {:error, _} ->
          {:noreply, put_flash(socket, :error, "Could not import — there may already be a recipe for that meal/date slot.")}
      end
    end
  end
    ...
    <div>
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <.link navigate={~p"/professional/ai-bot"} class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 border border-slate-700/60 text-sm transition-colors">
            ...
            <%= for recipe <- @untracked do %>
              <div class="flex items-center gap-3 px-4 py-3">
                ...
                <div class="flex items-center gap-1.5 flex-shrink-0">
                  <button phx-click="import_one" phx-value-id={recipe.id}
                  ...
                  <button phx-click="dismiss_one" phx-value-id={recipe.id}
          ...
          <%= for {date, recipes} <- @grouped do %>
            <div>
              <div class="flex items-center gap-3 mb-3">
                ...
                <span class="text-xs text-slate-600"><%= length(recipes) %> recipe<%= if length(recipes) != 1, do: "s" %></span>
              </div>
              <div class="space-y-2">
                <%= for recipe <- recipes do %>
                  <div class="bg-slate-800 border border-slate-700/60 rounded-xl p-4 flex items-center gap-4 hover:border-slate-600 transition-colors">
                    <%= if recipe.recipe.image_url do %>
                      <img src={recipe.recipe.image_url} class="w-14 h-14 rounded-lg object-cover flex-shrink-0" />
                    <% else %>
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                      <.link navigate={~p"/professional/ai-bot/review/#{recipe.id}"}
  ...
  defp meal_badge_class("morning_snack"), do: "bg-orange-500/20 text-orange-400 border-orange-500/30"
