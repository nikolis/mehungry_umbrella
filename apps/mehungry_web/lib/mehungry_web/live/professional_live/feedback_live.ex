  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-ink">
      <div class="container mx-auto px-4 py-6 max-w-3xl">
        <div class="mb-6">
          <h1 class="text-2xl font-bold text-white flex items-center gap-3">
            <.icon
              ...
              class="w-7 h-7 text-paprika flex-shrink-0"
            /> User Feedback
          </h1>
        ...
        <%= if @feedbacks == [] do %>
          <div class="bg-ink-panel border border-ink-panel2 rounded-xl p-12 text-center">
            <.icon name="hero-inbox" class="w-10 h-10 text-parchment-dim mx-auto mb-3" />
            <p class="text-parchment-dim">No feedback submitted yet.</p>
          </div>
        <% else %>
          <div class="space-y-3">
            <%= for fb <- @feedbacks do %>
                ...
                <div class="flex items-start justify-between gap-4">
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex flex-wrap items-center gap-3 mt-3">
                      <%= if fb.email do %>
                        <span class="flex items-center gap-1 text-xs text-parchment-dim">
                      ...
                      <%= if fb.user_id do %>
                        <span class="text-xs bg-paprika/10 text-paprika-soft border border-paprika/20 px-2 py-0.5 rounded-full">
                          registered user
                        </span>
                      <% else %>
                        <span class="text-xs bg-ink-panel2 text-parchment-dim border border-ink-panel2 px-2 py-0.5 rounded-full">
                          anonymous
                        </span>
                      <% end %>
                    </div>
                  ...
                  <button
                    ...
                    class="flex-shrink-0 p-1.5 rounded-lg text-parchment-dim hover:text-red-400 hover:bg-red-500/10 transition"
                  >
