  ...
  defp create_basket(socket, save_action, basket_params_params) do
    ...
    case changeset.valid? do 
      ...
      true ->  
        ...
        user_meals  = History.list_history_user_meals_for_user(socket.assigns.user.id, st_dt, en_dt)
        ...
        all_ingredients = 
         ...
         |> Map.values()
         |> Enum.map(fn {ing, quant, mu} ->  %{ingredient_id: ing, quantity: quant, measurement_unit_id: mu} end)
           

        IO.inspect(all_ingredients, label: "All Ingredeitns")
        params = %{start_dt: st_dt, end_dt: en_dt, basket_ingredients: all_ingredients, user_id: socket.assigns.user.id}
