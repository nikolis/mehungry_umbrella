defmodule MehungryWeb.BasketLive.FormComponeasfafsdafsdfdsdfasd do
  ...
  def update(%{shopping_basket: basket, user: user, patch: patch} = assigns, socket) do
    ...
    # IO.inspect(basket, label: "sdHPING BASKET IN UPDATE")
    {:ok,
     socket
     |> assign(assigns)
     |> assign(:user, user)
  ...
  defp create_basket(socket, basket_params_params) do
    ...
    case changeset.valid? do
      ...
      true ->
        ...
        shopping_basket = Inventory.update_shopping_basket(socket.assigns.shopping_basket, params)
